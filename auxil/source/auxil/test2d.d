...
unittest
{
	...
	model.header_size.should.be == width;
	model.i.size.should.be == width;
	model.f.size.should.be == width;
	model.size.should.be == 3*width;
	...
	() @trusted {
		...
		rm.output[i].path[].should.be == [1];
		rm.output[i].x.should.be == 2*width;
		rm.output[i].y.should.be == 0;
		rm.output[i].w.should.be == width;
		rm.output[i].h.should.be == height;
...
unittest
{
	...
	model[0].header_size.should.be == width;
	model[0].i.size.should.be == width;
	model[0].f.size.should.be == width;
	model[0].size.should.be == 3*width;

	{
		...
		auto expectedData = [
			TreePosition(0*width, 0, width, height, []),
			TreePosition(1*width, 0, width, height, [0]),
			TreePosition(2*width, 0, width, height, [0, 0]),
			TreePosition(3*width, 0, width, height, [0, 1]),
		];
...
unittest
{
	...
	model.header_size.should.be == height;
	model.header_size.should.be == 9;
...

	{
		...
		auto expectedData = [
			TreePosition( 0 + 0*width,     0, width, height, []), // TODO instead of width should be width
			TreePosition(15 + 0*width, height, width, height, [0]),
			TreePosition(15 + 1*width, height, width, height, [0, 0]),
			TreePosition(15 + 2*width, height, width, height, [0, 1]),
		];
...
unittest
{
	...
	model[0].header_size.should.be == height;
	model[0].i.size.should.be == height;
	model[0].f.size.should.be == height;
	model[0].size.should.be == 3*height;

	model[1].header_size.should.be == width;
	model[1].i.size.should.be == width;
	model[1].f.size.should.be == width;
	model[1].size.should.be == 3*width;

	model[2].header_size.should.be == height;
	model[2].i.size.should.be == height;
	model[2].f.size.should.be == height;
	model[2].size.should.be == 3*height;
	...
	() @trusted {
		...
		rm.output[i].path[].should.be == [1, 0];
		rm.output[i].x.should.be == 15 + width;
		rm.output[i].y.should.be == 40;
		rm.output[i].w.should.be == width;
		rm.output[i].h.should.be == height;
		i++;
		...
		rm.output[i].path[].should.be == [1, 1];
		rm.output[i].x.should.be == 15 + 2*width; // размер первого элемента плюс размер второго
		rm.output[i].y.should.be == 40;
		rm.output[i].w.should.be == width;
		rm.output[i].h.should.be == height;
		i++;
