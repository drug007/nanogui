...
writeln(__PRETTY_FUNCTION__, " ", focused);
        ...
        if (mEditable && focused)
        {
            if (action == KeyAction.Press || action == KeyAction.Repeat)
            {
                ...
                else if (key == Key.Right)
                {
                    ...
                    if (mCursorPos < cast(int) mValueTemp.length)
                        mCursorPos++;
                }
                ...
                else if (key == Key.End)
                {
                    ...
                    mCursorPos = cast(int) mValueTemp.length;
                }
                else if (key == Key.Backspace)
                {
                    if (!deleteSelection())
                    {
                        if (mCursorPos > 0)
                        {
                            mValueTemp.replaceInPlace(mCursorPos - 1, mCursorPos, (char[]).init);
                            mCursorPos--;
                        }
                    }
                }
                else if (key == Key.Delete)
                {
                    if (!deleteSelection())
                    {
                        if (mCursorPos < cast(int) mValueTemp.length)
                        {
                            mValueTemp.replaceInPlace(mCursorPos, mCursorPos+1, (char[]).init);
                        }
                    }
                }
	...
	override bool keyboardCharacterEvent(dchar codepoint)
    {
        if (mEditable && focused)
        {
            deleteSelection();
            import std.conv : to;
            mValueTemp.replaceInPlace(mCursorPos, mCursorPos+1, codepoint.to!string);
            mCursorPos++;
